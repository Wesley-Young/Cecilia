...
fun Transferable.hasSupportedImagePayload(): Boolean {
...
fun Transferable.extractImageFilePaths(): List<Path> {
...
fun Transferable.extractImageFromClipboard(): Image? {
    if (isDataFlavorSupported(DataFlavor.imageFlavor)) {
...
data class ClipboardImageBytes(
    val bytes: ByteArray,
    val formatHint: String?
)

fun Transferable.extractBinaryImages(): List<ClipboardImageBytes> {
